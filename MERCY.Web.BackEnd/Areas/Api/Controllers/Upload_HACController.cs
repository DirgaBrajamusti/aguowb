...
{
    public class Upload_HACController : Controller
    {
        public JsonResult Index()
        {
            return Json("UPLOAD_HACController", JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult ChooseFile()
        {
            ...
            if ( ! OurUtility.Upload_Record(user, fileName, fileName2, OurUtility.UPLOAD_HAC, ref id_File_Physical, ref msg))
            {
                var msgX = new { Success = false, Permission = permission_Item, Message = msg, MessageDetail = string.Empty, Version = Configuration.VERSION, Id = -1 };
                return Json(msgX, JsonRequestBehavior.AllowGet);
            }
            ...
            try
            {
                using (MERCY_Ctx db = new MERCY_Ctx())
                {
                    ...
                    using (SpreadsheetDocument document = SpreadsheetDocument.Open(UploadFolder + fileName, false))
                    {
                        ...
                        string date_Sampling = string.Empty;

                        TEMPORARY_HAC_Header header = null;
                        TEMPORARY_HAC detail = null;
                        ...
                        foreach (Sheet sheet in wbPart.Workbook.Sheets)
                        {
                            ...
                            try
                            {
                                ...
                                if (string.IsNullOrEmpty(ExcelProcessing.GetCellValue(wbPart, wsPart, "B9"))) continue;

                                header = new TEMPORARY_HAC_Header
                                {
                                    File_Physical = id_File_Physical,
                                    Sheet = sheetName,
                                    Company = company,
                                    Date_Detail = ExcelProcessing.GetCellValue(wbPart, wsPart, "L7"),
                                    Job_No = ExcelProcessing.GetCellValue(wbPart, wsPart, "B9").Replace(": ", ""),
                                    Report_To = ExcelProcessing.GetCellValue(wbPart, wsPart, "B10").Replace(": ", ""),
                                    Method1 = ExcelProcessing.GetCellValue(wbPart, wsPart, "B11").Replace(": ", ""),
                                    Method2 = ExcelProcessing.GetCellValue(wbPart, wsPart, "B12").Replace(": ", ""),
                                    Method3 = ExcelProcessing.GetCellValue(wbPart, wsPart, "B13").Replace(": ", ""),
                                    Method4 = ExcelProcessing.GetCellValue(wbPart, wsPart, "B14").Replace(": ", ""),
                                ...
                                db.TEMPORARY_HAC_Header.Add(header);
                                db.SaveChanges();
                                ...
                                while (true)
                                {
                                    date_Sampling = ExcelProcessing.GetCellValue(wbPart, wsPart, "A" + baris.ToString());
                                    if (string.IsNullOrEmpty(date_Sampling)) break;
                                    if (date_Sampling == "Cummulative Result") break;

                                    detail = new TEMPORARY_HAC
                                    {
                                        Header = header.RecordId,

                                        Date_Sampling = ExcelProcessing.GetDate(date_Sampling, "yyyy-MM-dd", ""),
                                        Day_work = ExcelProcessing.GetCellValue(wbPart, wsPart, "B" + baris.ToString()),
                                    ...
                                    if (detail.Date_Sampling_isvalid
                                ...
                                try
                                {
                                    ...
                                    // yang Status: New
                                    List<TEMPORARY_HAC> recordSet = (
                                                                                from dt in db.TEMPORARY_HAC
                                                                                orderby dt.RecordId
            ...
            try
            {
                using (MERCY_Ctx db = new MERCY_Ctx())
                {
                    var dataQuery =
                            (
                                from dt in db.TEMPORARY_HAC
                                join h in db.TEMPORARY_HAC_Header on dt.Header equals h.RecordId
                                orderby dt.RecordId
                                where h.File_Physical == id
                                select new Model_View_TEMPORARY_HAC
                                {
                                    Status = dt.Status
            ...
            try
            {
                using (MERCY_Ctx db = new MERCY_Ctx())
                {
                    using (var connection = new SqlConnection(db.Database.Connection.ConnectionString))
                    {
                        ...
                        command.CommandText = string.Format(@"exec UPLOAD_ACTUAL_HAC {0}", id);
        ...
        public static void Check_Validation(TEMPORARY_HAC p_detail, List<string> p_keys_Unique)
        {
                    ...
                    using (var connection = new SqlConnection(db.Database.Connection.ConnectionString))
                    {
                        ...
                        command.CommandText = string.Format(@"
                                                            ...
                                                            from TEMPORARY_File t
                                                                left join TEMPORARY_HAC_Header h on t.RecordId = h.File_Physical
                                                            where t.RecordId = {0}
                                                            ", file_Id);
