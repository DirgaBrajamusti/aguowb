IF OBJECT_ID('PROCESS_LECO_for_Sampling_ROM') IS NULL
    EXEC('CREATE PROCEDURE PROCESS_LECO_for_Sampling_ROM AS SET NOCOUNT ON;')
GO

alter procedure [dbo].[PROCESS_LECO_for_Sampling_ROM] @p_timeStamp datetime, @p_leco_Stamp varchar(255) output
as
begin
/*
...
*/
    ...
    BEGIN TRY
        ...
        BEGIN
            -- buat Nama dengan pola: ROM.TCM.XXX
            select @name = 'ROM.' + @company + '.' + @id
            ...
            BEGIN TRY
                ...
                begin
                    ...
                    set CV = CAST(ROUND(@value_CV, 0, 1) AS INT)
